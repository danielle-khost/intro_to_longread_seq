...
- *canu_ecoli_assemble_grid.slurm*: as above, but will run on the grid (i.e. Odyssey). By default, Canu will auto-detect available resources and will configure itself; however, it does not request explicit time limits or partitions. You can specify these using the gridOptions=" " argument, and can also control resources used by individual steps (e.g. correction, overlapping, etc).
- *miniasm_ecoli_assemble.slurm*: performs overlapping and assembly using minimap2 and miniasm, plus self-correction of the assembly using Racon. This should run quickly, but is error-prone: further correction with Illumina reads, or pre-correction of ONT reads  
